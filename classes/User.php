...
class User {
	...
	public function getName() {
		global $system;
		if (! isset ( $this->name )) {
			$statement = $system->getPdo()->prepare('SELECT name FROM user WHERE id = :id');
			$statement->bindValue(':id', $this->id, PDO::PARAM_INT);
			$statement->execute();
	...
	public function identification($name, $password) {
		global $system;
		$statement = $system->getPdo ()->prepare ( 'SELECT id FROM user WHERE name = :name AND password = :password' );
